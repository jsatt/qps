...
//	where=pstable->clickedColumn();
	...
	if(treeview and cats.size()>0 and cats[0]->id== F_CMD) // if command exists
	{
		if(where==0) where=1; // this problem
	}
...
void Procview::moveColumn(int col, int place) 
{
	...
	if( treeview==true )  
	{
		...
		if(place==0)	return;
		if(cats[col]->index==F_CMD) place=0; 
	}
	...
	if(place<col) col++;
	cats.remove(col);	 //second  remove idx
...
//		2.void Procview::set_fields()
void Procview::fieldArrange()
{

	if(treeview) {
		// Tree Mode 
		saveCOMMANDFIELD(); // save 
	
		// If COMMAND isn't the leftmost column, move it to leftmost
		if(idxF_CMD>=0) 	cats.remove(idxF_CMD); //removeField(F_CMD);
		cats.insert(0,categories[F_CMD]);
	...
	{
		//Linear_Mode:
		if(cats[0]->index==F_CMD) 	cats.remove(0); //removeField(F_CMD);

		//recover COMMAND field
		if(idxF_CMD>=0)
		{
			//printf(".a.a.\n");
			addField(F_CMD,idxF_CMD);
		}	
