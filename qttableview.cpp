/**********************************************************************
** $Id: qttableview.cpp,v 1.115.2.1 2011/08/28 07:07:19 fasthyun Exp $
...
void QtTableView::repaintCell( int row, int col, bool usecache ) // false  
{
	...
	view->repaint( uR.intersect(viewRect())); // slow
...
//	viewport()->setAutoFillBackground (false);
	...
	if ( xPos <= maxX ) {
		...
		r.setLeft( xPos );
		r.setBottom( yPos<maxY ? yPos : maxY );		
		view->repaint(r.intersect( updateR ));
	}
	if ( yPos <= maxY ) {  
		QRect r = viewR;
		r.setTop( yPos );
		view->repaint(r.intersect( updateR ) );	// why? CPU +2~3% -> rect.unite() 
	}
